(function() {
    ...
    var init = {
        start: function(id) {									//start: is like  'this.start='
            this.defaultProperties();						//this function only initializing the SCALE with 30
            ...
            this.callbacks();
            // main thing is  that every add.random is adding a body/shape into the world
            setTimeout(function() { add.random(); }, 100);      
            setTimeout(function() { add.random(); }, 2000);     //and the time(2000) given is the interval time,means after 2000ns this add.random will call.
        ...
        defaultProperties: function() {					//Set the SCALE = 30. Meaning that all pixel values are divide with 30 to get the size in meters. E.g. the canvas is 740 / 30 = 24.6 m.
            SCALE = 30;
        },
        canvas: function(id) {
        ...
        callbacks: function() {
            canvas.addEventListener('click', function(e) {     //where we click on the canvas its e.offsetX,e.offsetY can be captured via it 
                var shapeOptions = {
    ...
    var loop = {
        step: function() {
            var stepRate = 1 / 60;    //speed of droping the rigid body
            world.Step(stepRate, 10, 10);    //dont what is it doing   
			//world.Step(timeStep, velocityIterations, positionIterations);
            world.ClearForces();				//dont know what it is   answer is on stackoverflow
			//backdays when we use ApplyForces on an object it remain indefinitely,so for removing it we used world.ClearForces,but now engine use ClearForces after every step itself,so we dont need to use it.
        },
        update: function () {            
            for (var b = world.GetBodyList(); b; b = b.m_next) {    // b = b.getNext()
                if (b.IsActive() && typeof b.GetUserData() !== 'undefined' && b.GetUserData() != null) {
                    shapes[b.GetUserData()].update(box2d.get.bodySpec(b));			//shapes is gloabal dictionary,in this we have every body ,which need to draw every time
                }
            }
        },
        draw: function() {            
